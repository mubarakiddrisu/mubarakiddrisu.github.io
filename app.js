...
function courseGroup(group, current = false) {
  ...
  for (const course of group.courses) {
    ...
    const title = element('strong', 'course-title');
    const href = safeUrl(course.url);
    if (href) {
      const anchor = setLink(element('a', '', course.title), href);
      if (course.linkLabel) anchor.title = course.linkLabel;
      title.append(anchor);
    }
    else title.textContent = course.title;
    item.append(title);
    if (course.term) item.append(element('span', 'course-meta', course.term));
    list.append(item);
  }
...
function renderHome(data) {
  ...
  bio.replaceChildren(...data.biography.map(text => {
    const paragraph = element('p');
    const matches = Object.values(data.advisors || {})
      .filter(advisor => advisor.name)
      .map(advisor => ({advisor, index: text.indexOf(advisor.name)}))
      .filter(match => match.index >= 0)
      .sort((a, b) => a.index - b.index);
    let cursor = 0;
    for (const {advisor, index} of matches) {
      if (index < cursor) continue;
      paragraph.append(text.slice(cursor, index));
      const href = safeUrl(advisor.url);
      const name = element(href ? 'a' : 'span', '', advisor.name);
      if (href) setLink(name, href);
      paragraph.append(name);
      cursor = index + advisor.name.length;
    }
...
function renderResearch(data) {
  ...
  for (const group of data.research) {
    ...
    for (const paper of group.papers) {
      ...
      if (href) title.append(setLink(element('a', '', paper.title), href));
